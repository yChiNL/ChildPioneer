...
def onefoot_Ther(user, mode,end, id, background_path):
    global flipped_image, center_point
    from ChildPioneer.models import CustomUser, TreatmentContent
    from ChildPioneer.models import TrainingOneFoot
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if train_started:
                if (Mode == "VeryEasy_Left"):
                    ...
                    # 成功
                    success_direction = -1
                    doAction(f'成功次數: {move_count} / {end_count}', font=font_time, font_size=font_size_time,point=[170, 0])
        ...
        def import_data(user, move_count, fail_count, t_time, difficulty, grade):
            training_data = TrainingOneFoot(user=user, move_count=move_count, fail_count=fail_count, t_time=t_time,difficulty=difficulty, grade=grade)
...
def main(user, mode, end, id, background_name):
    ...
    try:  # server端
        ...
        background_path = '../media/' + background_name
        open(background_path, 'r+')
    thread = Thread(target=onefoot_Ther, args=(user, mode, end, id, background_path), daemon=True)
    thread.start()
...
if __name__ == '__main__':
    main('stest', 'VeryEasy_Left', 5, 0, 'background_images/remove.png')
