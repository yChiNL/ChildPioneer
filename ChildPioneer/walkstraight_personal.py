...
def walkstraight_Per(user, mode, background_path):
    global flipped_image, time_started, time1, failure_count, end_point, target_point, distance_cm, foot_distance, font_time, font, NormalMode1, NormalMode2, mp_drawing, mp_pose, results
    from ChildPioneer.models import TrainingWalkStraight, CustomUser
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        def import_data(user, move_count, failure_count, t_time, difficulty, grade):
            training_data = TrainingWalkStraight(user=user, move_count=move_count, failure_count=failure_count, t_time=t_time, difficulty=difficulty,grade=grade)
            training_data.save()
...
def main(user, mode):
    ...
    dance_thread = Thread(target=walkstraight_Per, args=(user, mode, background_path), daemon=True)
    dance_thread.start()
