...
def readImage(path):
    ...
    return image
def dance_Ther(user, mode, end, id, background_path):
    global flipped_image, center_point, font_size, font_size_time, move_count, m_started, time_started, font, font_time, state

    from pythonProject3.models import TrainingDance
    from pythonProject3.models import TreatmentContent, CustomUser
    ...
    right_elbow_y = None
    state = False
    movement = ['Dance_LeftFly', 'Dance_RightFly', 'Dance_FlFl', 'Dance_UpUp', 'Dance_DoDo', 'Dance_UpDo', 'Dance_DoUp', 'Dance_UpUp_wrist', 'Dance_DoDo_wrist', 'Dance_UpDo_wrist',
                'Dance_DoUp_wrist']
    ...
    time_point = (0,400)
    # target_point = (int(image_size[0]/2), image_size[1]-40)
    ...
    def hint(time1):
        ...
        cv2.putText(flipped_image, str(int(hint_time)) + 'sec', time_point, cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (0, 255, 0), 2)
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if train_started:
                ...
                if (Mode == "VeryEasy"):
                    ...
                    doAction(f"成功次數 {move_count} / {end_count}", point=[150, 410], font=font_time, font_size=font_size_time)
                    if time_started:
                        time_limit_minutes = 6  # 時間限制（分鐘）
                        time_limit_seconds = time_limit_minutes * 60  # 轉換為秒
                        time2 = time.time()
                    ...
                    if not m_started:
                        ...
                        if time_started:
                            ...
                            if int(time_count) == 3:
                                ...
                                # cv2.putText(flipped_image, movement[move], (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 255, 0), 2)
                ...
                if (Mode == "VeryHard"):
                    ...
                    if m_started:
                        ...
                        if movement[move] == 'Dance_UpUp_wrist':
                            ...
                            # 用於取得手掌位置的code
                            l_palm_x = int(landmarks[mp_pose.PoseLandmark.RIGHT_WRIST].x * flipped_image.shape[1])
                            l_palm_y = int(landmarks[mp_pose.PoseLandmark.RIGHT_WRIST].y * flipped_image.shape[0])
                            r_palm_x = int(landmarks[mp_pose.PoseLandmark.LEFT_WRIST].x * flipped_image.shape[1])
                            r_palm_y = int(landmarks[mp_pose.PoseLandmark.LEFT_WRIST].y * flipped_image.shape[0])
        ...
        if Mode == "VeryEasy":
            if move_count == end_count and t_time <= 60:
                grade = 'A'
            elif move_count == end_count and t_time > 60 and t_time <= 100:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "Easy":
            if move_count == end_count and t_time <= 100:
                grade = 'A'
            elif move_count == end_count and t_time > 100 and t_time <= 140:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "Normal":
            if move_count == end_count and t_time <= 140:
                grade = 'A'
            elif move_count == end_count and t_time > 140 and t_time <= 180:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "Hard":
            if move_count == end_count and t_time <= 180:
                grade = 'A'
            elif move_count == end_count and t_time > 180 and t_time <= 220:
                grade = 'B'
            else:
                grade = 'C'

        elif Mode == "VeryHard":
            if move_count == end_count and t_time <= 220:
                grade = 'A'
            elif move_count >= end_count and t_time > 220 and t_time <= 260:
                grade = 'B'
            else:
                grade = 'C'
        # 評分機制
        print(f"在 {Mode} 模式下，您的評分是 {grade}")
...
def main(user, mode, end, id, background_name):
    ...
    try:  # server端
        ...
        open(background_path, 'r+')
    thread = Thread(target=dance_Ther, args=(user, mode, end, id, background_path), daemon=True)
    thread.start()
...
if __name__ == '__main__':
    main('stest', 'VeryEasy', 3, 0, 'remove.png')
