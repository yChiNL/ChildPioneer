...
def jump_Per(user, mode, background_path):
    from pythonProject3.models import CustomUser
    from pythonProject3.models import TrainingJump
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if abs(target_x - target_point[0]) < 10 and abs(target_y - target_point[1]) < 10:
                    ...
                    if show_text:
                        ...
                        draw.text(calculateTextPoint("請站在點上", center_point, font_size), "請站在點上",
            ...
            if train_started:
                ...
                if (Mode == "VeryEasy"):
                    ...
                    pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_image)
                    draw.text((25, 50), "成功次數 " + str(move_count) + "/" + str(3), fill=(0, 255, 0), font=font_time)
                    flipped_image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)
                    img = flipped_image
                    ...
                    if mouse_move_easy:
                        ...
                        if abs(target_b < goal_point[1]) and abs(target_a < goal_point[1]):
                            if not success_displayed:
                                ...
                                # 读取带有透明通道的图像
                                another_success_image = cv2.imread('media/Successmouse.png', cv2.IMREAD_UNCHANGED)
                ...
                if (Mode == "Easy"):
                    ...
                    pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_image)
                    draw.text((25, 50), "成功次數 " + str(move_count) + "/" + str(5), fill=(0, 255, 0), font=font_time)
                    flipped_image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)
                    img = flipped_image
                    ...
                    if mouse_move_easy:
                        ...
                        draw.text(calculateTextPoint("跳過貓咪!", center_point, font_size), "跳過貓咪!",
                                  fill=(0, 255, 0), font=font)
                        mouse_image = readImage('media/cat.png')  # 以彩色模式读取老鼠图像
                        re_image = cv2.resize(mouse_image[:], (100, 100))
                ...
                if (Mode == "Normal"):
                    ...
                    pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_image)
                    draw.text((25, 50), "成功次數 " + str(move_count) + "/" + str(8), fill=(0, 255, 0), font=font_time)
                    flipped_image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)
                    img = flipped_image
                    ...
                    if mouse_move_easy:
                        ...
                        draw.text(calculateTextPoint("跳過長頸鹿!", center_point, font_size), "跳過長頸鹿!",
                                  fill=(0, 255, 0), font=font)
                        flipped_image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)
                        mouse_image = readImage('media/234.png')  # 以彩色模式读取老鼠图像
                        re_image = cv2.resize(mouse_image[:], (100, 100))
                ...
                if (Mode == "Hard"):
                    ...
                    pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_image)
                    draw.text((25, 50), "成功次數 " + str(move_count) + "/" + str(10), fill=(0, 255, 0), font=font_time)
                    flipped_image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)

                    if move_count == 10:  # 之後傳送數據
                        pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                        draw = ImageDraw.Draw(pil_image)
                        draw.text((150, 150), "結束", fill=(0, 255, 0), font=font)
                        t_time2 = time.time()
                    ...
                    if Jump_Right:
                        ...
                        if mouse_move and not fg:
                            mouse_image = readImage('media/mouse.png')  # 以彩色模式读取老鼠图像
                            re_image = cv2.resize(mouse_image[:], (100, 100))
                ...
                if (Mode == "VeryHard"):
                    ...
                    pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                    draw = ImageDraw.Draw(pil_image)
                    draw.text((25, 50), "成功次數 " + str(move_count) + "/" + str(12), fill=(0, 255, 0), font=font_time)
                    flipped_image = cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)

                    if move_count == 12:  # 之後傳送數據
                        pil_image = Image.fromarray(cv2.cvtColor(flipped_image, cv2.COLOR_BGR2RGB))
                        draw = ImageDraw.Draw(pil_image)
                        draw.text((150, 150), "結束", fill=(0, 255, 0), font=font)
                        t_time2 = time.time()
        ...
        if Mode == "VeryEasy":
            if move_count == 3 and t_time <= 60:
                grade = 'A'
            elif move_count == 3 and t_time > 60 and t_time <= 120:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "Easy":
            if move_count == 5 and t_time <= 100:
                grade = 'A'
            elif move_count == 5 and t_time > 100 and t_time <= 200:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "Normal":
            if move_count == 8 and t_time <= 120:
                grade = 'A'
            elif move_count == 8 and t_time > 120 and t_time <= 180:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "Hard":
            if move_count == 10 and t_time <= 140:
                grade = 'A'
            elif move_count == 10 and t_time > 140 and t_time <= 220:
                grade = 'B'
            else:
                grade = 'C'
        elif Mode == "VeryHard":
            if move_count == 12 and t_time <= 160:
                grade = 'A'
            elif move_count >= 12 and t_time > 160 and t_time <= 240:
                grade = 'B'
            else:
                grade = 'C'
        # 打印评分
...
def main(user, mode, background_name):
    ...
    try:  # server端
        ...
        background_path = '../media/' + background_name
        open(background_path, 'r+')
    thread = Thread(target=jump_Per, args=(user, mode, background_path), daemon=True)
    thread.start()
...
if __name__ == '__main__':
    main('stest', 'VeryEasy', 'background_images/remove.png')